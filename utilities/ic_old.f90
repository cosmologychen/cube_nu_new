...
program initial_conditions
  ...
  call system_clock(t1,t_rate)
  open(11,file='./tf/camb_32288000_transfer_out_z0.dat.txt',form='formatted')
  read(11,*) tf
  close(11)
...
# ifdef READ_NOISE
    ...
    read(11) rho1
    close(11)
    if (head .or. this_image()==num_images()) print*, '  READ IN NOISE MAP:', rho1(1,1,1), rho1(ng,ng,ng)
# else
...
stop
  ...
  sim%vz_max=vmax(3)
  nlayer=2*ceiling(grad_max(3)*np_nc/8/pi/ratio_cs)+2 ! for OpenMP
  if (head) then
