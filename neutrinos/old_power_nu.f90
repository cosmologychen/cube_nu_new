...
!         dFy = 0.0
 ...
 !     endif
   ...
   function get_sqrt_pk_nu1() result(spk)
      ...
      print*,'========================================='
      print*,'  [get_sqrt_pk_nu1] 标量变量输出：'
      print*,'  a_step(0):',a_step(0)
      print*,'  omega_m_zi（红移处物质密度参数）：', omega_m_zi
      print*,'  omega_l_zi（红移处暗能量密度参数）：', omega_l_zi
      print*,'  f：', f
      print*,'========================================='

      ! ========== 二维数组X关键元素输出（避免全输出冗余） ==========
      print*,'  [get_sqrt_pk_nu1] 数组X关键元素输出：'
      print*,'  X(1,1)：', X(1,1)
      print*,'  X(1,2)：', X(1,2)
      print*,'  X(1,3)：', X(1,3)
      print*,'========================================='
   ...
   function get_sqrt_pk_nu2() result(spk)
      ...
      spk = k2/3 * (tau_step(1)**3-tau_step(0)**3) + (b2/2 * (tau_step(1)+tau_step(0)) + c2)*(tau_step(1)-tau_step(0))
