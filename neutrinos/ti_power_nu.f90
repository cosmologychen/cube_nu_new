...
!         dFy = 0.0
 ...
 !     endif
   ...
   function get_sqrt_pk_nu2() result(spk)
      ...
      implicit none
      real spk(npbin,3)
      real(8) L_prev(npbin,3), L_curr(npbin,3)  ! 仅保留相邻两步的L值，用于梯形积分
      integer is

      ! 初始化spk为0，避免未初始化问题
      spk = 0.0d0

      ! 获取前两步的L值（对应tau_step(0)和tau_step(1)）
      L_prev = get_L(0)  ! 前一步L值（对应tau_step(is-1)）
      L_curr = get_L(1)  ! 当前步L值（对应tau_step(is)）

      ! 处理istep=1的特殊情况（仅两个点，直接计算梯形面积）
      if (istep == 1) then
         ! 梯形积分公式：(上底+下底)*高/2
         spk = (L_prev + L_curr) * (tau_step(1) - tau_step(0)) * 0.5d0
         return
      endif

      ! 第一步：先累加tau_step(0)到tau_step(1)的梯形积分（对应istep=1）
      spk = spk + (L_prev + L_curr) * (tau_step(1) - tau_step(0)) * 0.5d0

      ! 迭代累加：从is=2到istep，计算每一段相邻tau的梯形积分
      do is = 2, istep
         ! 更新L值：前一步L值 = 上一轮当前L值；当前L值 = 最新get_L(is)
         L_prev = L_curr
         L_curr = get_L(is)

         ! 梯形积分：累加当前段（tau_step(is-1) 到 tau_step(is)）的面积
         spk = spk + (L_prev + L_curr) * (tau_step(is) - tau_step(is-1)) * 0.5d0
      enddo

      ! 原代码末尾额外的二次积分项无需保留（梯形积分已在循环内完成全部累加）
   endfunction
   ...
   subroutine interp_Pk_CDM
      ...
      interp_powerpoint = merge(sim%cur_powerpoint-1,3,sim%cur_powerpoint > 2)
